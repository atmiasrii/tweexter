...
class EnhancedPersonaEngine:
    ...
    def _basic_content_match(self, tweet, persona):
        """Fallback content matching without NLP"""
        tweet_lower = tweet.lower()
        
        # Simple keyword matching
        interest_matches = sum(1 for interest in persona.interests if interest.lower() in tweet_lower)
        trigger_matches = sum(1 for trigger in persona.triggers if trigger.lower() in tweet_lower)
        content_matches = sum(1 for content in persona.content_types if content.lower() in tweet_lower)
    ...
    def set_trending_hashtags(self, hashtags=None):
        ...
        if hashtags is not None:
            ...
            print("⚠️ No trending hashtags available - provide hashtags manually or install trending_hashtags module")
...
def load_personas(json_path):
    ...
    try:
        ...
        print(f"Warning: {json_path} not found. Creating minimal personas for testing.")
        data = create_sample_personas_if_missing(json_path)
    except json.JSONDecodeError:
        print(f"Warning: {json_path} contains invalid JSON. Using minimal personas.")
        data = []
    
    if not data:
        print("Warning: No persona data available. Using default personas.")
        return []
    ...
    for p in data:
        ...
        content_types = set(behavior.get('content_type', []))
        content_triggers = set(engagement.get('content_triggers', []))
        emotional_triggers = set(engagement.get('emotional_triggers', []))
        dealbreakers = set(engagement.get('dealbreakers', []))
        pet_peeves = set(behavior.get('pet_peeves', []))

        personas.append(Persona(
            id=p.get('id', len(personas)),
            interests=set(psych.get('interests', [])),
            triggers=content_triggers | set(psych.get('triggers', [])),
...
def simulate_persona_engagement(tweet_text, persona, debug=False):
    if debug:
    ...
    # Preprocess tweet: lowercase, split words, keep phrases
    words = {word.strip('.,!?":;').lower() for word in tweet_text.split()}
    phrases = {phrase.strip().lower() for phrase in re.split(r'[.,!?;:\n]+', tweet_text) if phrase.strip()}
    ...
    base_engagement = {
        ...
        'proactive': {'like': 0.05, 'retweet': 0.02, 'reply': 0.01},
        'influencer': {'like': 0.08, 'retweet': 0.03, 'reply': 0.015}
    }.get(persona.engagement_style, {'like': 0.03, 'retweet': 0.01, 'reply': 0.005})

    # Check for dealbreakers and pet peeves
    if (any(peeve.lower() in tweet_text.lower() for peeve in persona.pet_peeves) or
        any(dealbreaker.lower() in tweet_text.lower() for dealbreaker in persona.dealbreakers)):
        if debug:
            print("❌ Dealbreaker/pet peeve detected - zero engagement")
        return {'like': 0, 'retweet': 0, 'reply': 0}

    # Enhanced content-type matching with fuzzy logic
    content_match = 1.0
    exact_matches = sum(1 for topic in persona.content_types if topic.lower() in tweet_text.lower())
    if exact_matches > 0:
        content_match = 1.0 + (exact_matches * 0.3)  # 30% boost per match
    
    # Fuzzy content matching for partial matches
    partial_matches = 0
    for content_type in persona.content_types:
        for word in words:
            try:
                if fuzz.partial_ratio(content_type.lower(), word) > 70:
                    partial_matches += 1
                    break
            except:
                # Fallback to basic substring matching
                if content_type.lower() in word:
                    partial_matches += 1
                    break
    ...
    if debug:
        ...
        print(f"Emotion match: {emotion_match:.2f}")
        print(f"Interest match: {interest_match:.2f} (overlaps: {interest_overlap})")
        print(f"Trigger match: {trigger_match:.2f} (overlaps: {trigger_overlap})")
        print(f"Style modifier: {style_modifier:.2f}")
    ...
    if debug:
        print(f"Final engagement: L:{like_prob:.3f} RT:{retweet_prob:.3f} R:{reply_prob:.3f}")
...
def aggregate_engagement(tweet_text, personas, demographic_weights=None):
    ...
    for persona in personas:
        engagement = simulate_persona_engagement(tweet_text, persona)
        weight = demographic_weights.get(persona.id, 1.0) if demographic_weights else 1.0
        total_likes += engagement['like'] * weight
        total_rts += engagement['retweet'] * weight
        total_replies += engagement['reply'] * weight
        ...
        if engagement['like'] > 0.02 or engagement['retweet'] > 0.008 or engagement['reply'] > 0.005:
            ...
            if 'match_factors' in engagement:
                for factor, value in engagement['match_factors'].items():
    ...
    # Demographic insights with relative threshold
    engagements = [(persona, simulate_persona_engagement(tweet_text, persona)) for persona in personas]
    ...
    engaged_personas = []
    for persona, engagement in sorted_personas:
        # Much lower thresholds to actually capture engaged personas
        if engagement.get('like', 0) > 0.02 or engagement.get('retweet', 0) > 0.008 or engagement.get('reply', 0) > 0.005:
            engaged_personas.append({
                'id': persona.id,
                'demographic': persona.demographic,
                'engagement': engagement
            })
    engaged_personas = sorted(engaged_personas, key=lambda x: x['engagement']['like'], reverse=True)[:20]
    return {
...
def run_elo_tournament(tweet_variants, personas, demographic_weights=None):
    ...
    # Pre-calculate engagement for efficiency
    for tweet in tweet_variants:
        engagement_data[tweet] = aggregate_engagement(
...
class PersonaEngineAuditor:
    ...
    def _load_personas(self, path):
        ...
        try:
            ...
            print(f"Warning: {path} not found. Using empty persona list.")
            return []
        except json.JSONDecodeError:
            print(f"Warning: {path} contains invalid JSON. Using empty persona list.")
            return []
    ...
    def log_predictions(self, tweet_text, actual_engagement=None):
        ...
        print(f"DEBUG: Found {len(engaged_personas)} engaged personas for tweet: {tweet_text[:50]}...")
        if len(engaged_personas) > 0:
            print(f"DEBUG: First engaged persona: ID={engaged_personas[0].get('id')}, "
        ...
        print(f"DEBUG: Found {len(top_segments)} top segments")
        if len(top_segments) > 0:
            print(f"DEBUG: First segment: {top_segments[0].get('segment', 'Unknown')} "
...
def create_sample_personas_if_missing(path="personas.json"):
    ...
    if not os.path.exists(path):
        print(f"Creating sample personas file at {path}")
        sample_personas = [
            {
                "id": 1,
                "demographics": {
